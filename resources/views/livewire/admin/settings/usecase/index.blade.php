        ...
        <div class="d-block mb-4 mb-md-0">
            <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
                <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ __('UseCase List') }}</li>
                </ol>
            </nav>
            <h2 class="h4">{{ __('UseCase List') }}</h2>
            <p class="mb-0">{{ __('Show All UseCase List') }}</p>
        </div>
        <div class="btn-toolbar mb-2 mb-md-0">
            <a href="{{ route('admin.usecase.create') }}" class="btn btn-sm btn-gray-800 d-inline-flex align-items-center">
        ...
        <div class="row justify-content-between align-items-center">
            <div class="col-9 col-lg-8 d-md-flex">
                <div class="input-group me-2 me-lg-3 fmxw-300">
                    ...
                    <input type="text" class="form-control" wire:model="search" placeholder="Search UseCases" />
                </div>
                    ...
                    <tr>
                        <td>
                            <div class="form-check dashboard-check">
                                <input class="form-check-input" type="checkbox" wire:model="checklistId" value="{{ $useCase->id }}" />
                            </div>
                        </td>
                        <td>
                            <span class="fw-bold">{{ $useCase->name }}</span>
                        </td>
                        <td><span class="fw-normal">{{ $useCase->created_at->isoFormat('ll') }}</span></td>
                        <td>
                            @if ($useCase->status == 'approved')
                            <span class="fw-normal text-success">
                                {{ __('Active') }}
                            </span>
                            @elseif($useCase->status == 'pending')
                            <span class="fw-normal text-danger">
                                {{ __('Pending') }}
                            </span>
                            @elseif($useCase->status == 'draft')
                            <span class="fw-normal text-danger">
                                {{ __('Draft') }}
                            </span>
                            @endif
                        ...
                        <td>
                            <div class="btn-group text-center">
                                ...
                                <div class="dropdown-menu dashboard-dropdown dropdown-menu-start mt-2 py-1">
                                    <a class="dropdown-item d-flex align-items-center" href="{{ route('admin.usecase.edit',$useCase->id) }}">
